...
public class Player : Area2D {
	...
	public override void _PhysicsProcess(float delta) {
		...
		if (velocity.x > 0 && _animationPlayer.CurrentAnimation != "right" && !_jumping) _animationPlayer.Play("right");
		else if (velocity.x > 0 && velocity.y == 0) _animationPlayer.CurrentAnimation = "right";

		else if (velocity.x < 0 && _animationPlayer.CurrentAnimation != "left" && !_jumping) _animationPlayer.Play("left");
